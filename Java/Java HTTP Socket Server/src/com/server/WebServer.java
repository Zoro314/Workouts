...
public class WebServer {
			...
			public void run() {
				...
				try {
				  if( clientMap.get(requestBody.replaceAll("\\D+","")) != null ) {
                      ...
                      output.close();
                      socketToClose.close();
                      clientMap.remove(requestBody.replaceAll("\\D+", "")) ;  // removing the connection entry from the connection_store_Map
                     ...
                     outputForKill.println("Last-modified: Fri, 09 Aug 1996 14:21:40 GMT\r\n");
                     outputForKill.println("\r\n");
                      outputForKill.println("{status : invalid connection Id : < "+requestBody.replaceAll("\\D+", "")+" > }");
                      outputForKill.close();
                      clientSocket.close();
                  }
				} catch (IOException e) {
	...
	public void callMethod(WebServerRequestBody webServerRequestBody, Socket clientSocket) throws Exception {
        ...
        System.out.println("#######" + "The Control is here NOw !!!!!" + "#######");
		...
		if (WebServerUtils.isPostRequest(webServerRequestBody)) {
			...
			if (webServerRequestBody.getRequestUri() != null
				...
				System.out.println("Received a Sleep Request");
				implementSleepRequest(webServerRequestBody.getRequestParams(), clientSocket);
			} else {
				System.out.println("Received a Server Status Request");
